 ...
 * #L%
 */
package hu.icellmobilsoft.sampler.common.grpc.protoc.plugin;
...
/**
 * Quarkus generátor copy
 * https://github.com/quarkusio/quarkus/blob/main/extensions/grpc/protoc/src/main/java/io/quarkus/grpc/protoc/plugin/MutinyGrpcGenerator.java
 * @author Paulo Lopes
 *
 * A templateket cseréltem benne
 *
 * @author mark.petrenyi
 * @since 2.0.0
 */
public class CoffeeGrpcGenerator extends Generator {
    ...
    public List<PluginProtos.CodeGeneratorResponse.File> generateFiles(PluginProtos.CodeGeneratorRequest request) throws GeneratorException {
        ...
        List<DescriptorProtos.FileDescriptorProto> protosToGenerate = request.getProtoFileList().stream()
                .filter(protoFile -> request.getFileToGenerateList().contains(protoFile.getName())).collect(Collectors.toList());
        ...
        for (ServiceContext service : services) {
            if (service.packageName == null || service.packageName.isBlank()) {
                log.log(Level.SEVERE, "Using the default java package is not supported for "
                        + "Quarkus gRPC code generation. Please specify `option java_package = \"your.package\"` in " + service.protoName);
                failed = true;
            }
        }
        if (failed) {
    ...
    private List<PluginProtos.CodeGeneratorResponse.File> generateFiles(List<ServiceContext> services) {
        ...
        for (ServiceContext service : services) {
            files.add(buildFile(service, "ServiceInterface.mustache", absoluteFileName(service, service.serviceName + ".java")));
            files.add(buildFile(service, "ServiceImpl.mustache", absoluteFileName(service, service.delegatorImplName + ".java")));
        }
        return files;
    }
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            ...
            ProtocPlugin.debug(new CoffeeGrpcGenerator(), args[0]);
        }
    }
}
